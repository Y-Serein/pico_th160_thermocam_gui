...
CARD_TITLES = [
    ...
    ("img_dt",     "h − l  contrast"),
    ("gain",       "per-pixel gain"),
    ("bad points", "defect mask"),
    ("summary",    "stats · flash check"),
]
...
class CalibrateTab(QWidget):
    def __init__(self, parent=None):
        ...
        self.save_cb = QCheckBox("Auto-save PNG")
        ...
        ctrl.addWidget(QLabel("  Payload baud:")); ctrl.addWidget(self.baud_edit)
        ...
        for i, (name, subtitle) in enumerate(CARD_TITLES):
            ...
            if name == "summary":
                ...
                ax.text(0.03, 0.55,
                        ...
                        "This captures img_l / img_h / img_bg,\n"
                        "computes gain & bad-points, then writes\n"
                        "the result to on-device flash.",
                        transform=ax.transAxes, va='center', ha='left',
                        color='#8a90ab', fontsize=9, family='monospace')
            else:
                style_card(ax, name, subtitle)
                empty_placeholder(ax, msg='·  no data yet  ·')
    ...
    def _run(self):
        ...
        self.log.clear()
        self._log("Calibration started. This overwrites flash calibration.")
    ...
    def _on_success(self, img_l, img_h, img_bg, gain, badpts, bg_ok):
        ...
        self._log("-" * 40)
        self._log(f"  img_l   mean/max : {img_l.mean():.1f} / {img_l.max()}")
        self._log(f"  img_h   mean/max : {img_h.mean():.1f} / {img_h.max()}")
        self._log(f"  img_dt  mean     : {img_dt.mean():.1f}")
        self._log(f"  gain    min/max  : {gain.min():.4f} / {gain.max():.4f}")
        self._log(f"  gain    p1 / p99 : {gv1:.4f} / {gv99:.4f}")
        self._log(f"  bad points       : {len(filt_bp)}  {filt_bp}")
    ...
    def _render(self, fig, interactive):
        ...
        if interactive:
            ax1 = fig.add_subplot(2, 3, 1); style_card(ax1, "img_l", "cold shutter")
            ax2 = fig.add_subplot(2, 3, 2); style_card(ax2, "img_h", "hot shutter")
            ax3 = fig.add_subplot(2, 3, 3); style_card(ax3, "img_dt", "h − l  contrast")
            ax4 = fig.add_subplot(2, 3, 4); style_card(ax4, "gain", "per-pixel")
            ax5 = fig.add_subplot(2, 3, 5); style_card(ax5, "bad points", "defect mask")
            ax6 = fig.add_subplot(2, 3, 6); style_summary_card(ax6, "summary")
        else:
        ...
        # summary panel
        if interactive:
            status_badge(ax6, r['bg_ok'], "flash write check",
                         x=0.04, y=0.90)
            kv_block(ax6, [
                ('img_l  mean', f"{r['img_l'].mean():.1f}"),
                ('img_l  max',  f"{r['img_l'].max()}"),
                ('img_h  mean', f"{r['img_h'].mean():.1f}"),
                ('img_h  max',  f"{r['img_h'].max()}"),
                ('img_dt mean', f"{r['img_dt'].mean():.1f}"),
                ('gain   μ',    f"{r['gain'].mean():.4f}"),
                ('gain   σ',    f"{r['gain'].std():.4f}"),
                ('gain   p1',   f"{r['gv1']:.4f}"),
                ('gain   p99',  f"{r['gv99']:.4f}"),
                ('bad points',  f"{len(r['badpts'])}"),
    ...
    def _save_pngs(self):
        ...
        self._log(f"PNGs saved → {out_dir}")
